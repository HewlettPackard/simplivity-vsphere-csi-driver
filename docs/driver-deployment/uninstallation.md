...
Remove the Daemon Set, Deployment and RBAC for the HPE SimpliVity CSI Driver using the yaml files dowloaded earlier.

```text
$ kubectl delete -f svt-csi-node-ds.yaml
daemonset.apps "svt-csi-node" deleted

$ kubectl delete -f svt-csi-controller-deployment.yaml
deployment.apps "svt-csi-controller" deleted
csidriver.storage.k8s.io "csi.simplivity.hpe.com" deleted

$ kubectl delete -f svt-csi-controller-rbac.yaml
